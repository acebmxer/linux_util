...
# Add Local MOTD only for Ubuntu and KDE Neon
if [[ "$DISTRO_ID" == "ubuntu" || "$DISTRO_ID" == "neon" ]]; then
    SYSTEM_TASKS+=("Local MOTD")
...
get_version_qbittorrent() {
    qbittorrent --version 2>/dev/null | sed -n 's/.*v[0-9]*:\([0-9.]*\).*/v\1/p' || echo ""
}
