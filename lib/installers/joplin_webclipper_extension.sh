...
check_joplin_webclipper_extension() {
    ...
    for browser in "${!_JWC_CHROMIUM_POLICY_DIRS[@]}"; do
        ...
        [[ -f "$pf" ]] && grep -q "${_JWC_EXT_ID};${_JWC_EXT_UPDATE_URL}" "$pf" 2>/dev/null && return 0
    done
    local ff_path
    ff_path="$(_jwc_ext_firefox_policy_path)"
    [[ -n "$ff_path" && -f "$ff_path" ]] && grep -q "\"${_JWC_FF_GUID}\"" "$ff_path" 2>/dev/null && return 0
    local lw_path
    lw_path="$(_jwc_ext_librewolf_policy_path)"
    [[ -n "$lw_path" && -f "$lw_path" ]] && grep -q "\"${_JWC_FF_GUID}\"" "$lw_path" 2>/dev/null && return 0
    local zen_path
