...
install_amd_drivers() {
    ...
    case "$DISTRO_FAMILY" in
        debian)
            ...
            sudo apt update
            pkg_install mesa-vulkan-drivers:i386 libvulkan1:i386 2>/dev/null || warn "32-bit Vulkan libs not available on this system."
        ...
        fedora)
            ...
            if rpm -q rpmfusion-free-release &>/dev/null; then
                pkg_install mesa-vulkan-drivers.i686 vulkan-loader.i686 2>/dev/null || true
            fi
            ;;
        rhel)
        ...
        arch)
            ...
            if grep -q "^\[multilib\]" /etc/pacman.conf; then
                pkg_install lib32-mesa lib32-vulkan-radeon lib32-libva-mesa-driver lib32-mesa-vdpau 2>/dev/null || true
            fi
            ;;
        suse)
