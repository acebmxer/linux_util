...
setup_mount_nfs_share() {
    ...
    if [[ ! -d "$mount_point" ]]; then
        # Determine whether we need root to create the directory.
        # Walk up to find the first existing ancestor and check if it's writable.
        ...
        if [[ -w "$_ancestor" ]]; then
            ...
            # If the mount point is not inside the user's home dir, hand ownership
            # to the current user so they can browse it without sudo once mounted.
            if [[ "$mount_point" != "$HOME"* ]]; then
