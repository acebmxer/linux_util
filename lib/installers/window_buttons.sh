...
# close buttons.
#
# Why this exists: GNOME's default window-manager button layout omits
# minimize/maximize, so GTK apps like Remmina, Nautilus and Files show only a
# close button on a stock GNOME session. The window-manager preference below
# restores all three.
#
...
detect_window_button_de() {
    ...
    # No usable hint — probe for an available mechanism. Prefer the GNOME
    # schema since GNOME sessions are the most common case with an empty hint.
...
install_window_buttons() {
    # A GUI/session bus is required for these settings to take effect.
    if [[ -z "${DBUS_SESSION_BUS_ADDRESS:-}" ]] \
       && [[ -z "${WAYLAND_DISPLAY:-}" ]] \
       && [[ -z "${DISPLAY:-}" ]]; then
        warn "No graphical session detected (no D-Bus/Wayland/X display)."
        echo "  This setting only applies to a desktop session. Re-run this from"
        echo "  inside a graphical session (a native desktop login)."
        return 0
    fi
