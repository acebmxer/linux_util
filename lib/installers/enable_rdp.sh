...
# krdp:  KDE-native RDP server, Wayland-native (KDE Plasma 6+, recommended for Kubuntu 26.04+)
#
...
install_krdp() {
    ...
    info "krdp installed."
    info "To enable: open System Settings > Remote Desktop and turn on the RDP server."
    info "Or run:    systemctl --user enable --now plasma-krdp.service"
    return 0
}

uninstall_krdp() {
    info "Uninstalling krdp..."
    systemctl --user stop plasma-krdp.service 2>/dev/null || true
    systemctl --user disable plasma-krdp.service 2>/dev/null || true
