...
# whatever else is in the Start menu. Those directories sit ahead of /usr/bin
# on most PATHs, so a launcher shadows the Linux program of the same name.
# Detection must never mistake one for a local install, and must never run one
# to read a version: each launcher opens an RDP session to the VM, which costs
# seconds and flashes a Windows window on screen.

# Is this file a WinApps launcher? They are two-line scripts of a fixed shape,
# generated by upstream's setup.sh:
#     #!/usr/bin/env bash
#     /path/to/winapps <app> "$@"
# The shape is matched rather than the location, because WinApps drops these
# in the same directories a user's own scripts live in. 'read -N' caps the
# read at a fixed byte count so a large binary on PATH costs one short read.
_is_winapps_stub() {
    local _head
    [[ -f "$1" && -r "$1" ]] || return 1
    IFS= read -r -N 200 _head < "$1" 2>/dev/null
    [[ $_head == '#!'*"/winapps "*'"$@"'* ]]
}
...
# PATH is walked directly instead of shelling out, since this runs once per
# utility during startup detection.
_native_command() {
    local _cmd="$1" _dir _path
    local -a _dirs
    IFS=: read -ra _dirs <<< "$PATH"
    for _dir in "${_dirs[@]}"; do
        _path="${_dir:-.}/$_cmd"
        [[ -f "$_path" && -x "$_path" ]] || continue
        _is_winapps_stub "$_path" && continue
        printf '%s\n' "$_path"
        return 0
    done
    return 1
...
# Boolean form of _native_command: the drop-in replacement for
# "command -v foo &>/dev/null" in installation checks.
_have_cmd() {
...
_run_native() {
    local _bin
    _bin=$(_native_command "$1") || return 1
    shift
    "$_bin" "$@"
}
...
_ver_from_cmd() {
    local _cmd="$1" _flag="${2:---version}" _bin v
    # Run the resolved path, not the bare name: a WinApps launcher earlier on
    # PATH would otherwise be executed and start an RDP session.
    _bin=$(_native_command "$_cmd") || return 1
    v=$("$_bin" "$_flag" 2>/dev/null | grep -oP '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
    [[ -n "$v" ]] && printf '%s\n' "$v" || return 1
}
